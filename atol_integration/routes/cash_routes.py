...
async def cash_in(
    ...
    """
    return redis.execute_command('cash_in', request.model_dump())
...
async def cash_out(
    ...
    """
    return redis.execute_command('cash_out', request.model_dump())
...
async def get_cash_sum(redis: RedisClient = Depends(get_redis_client)):
    ...
    """
    return redis.execute_command('query_data', {'data_type': 3}) # LIBFPTR_DT_CASH_SUM = 3
...
async def open_cash_drawer(redis: RedisClient = Depends(get_redis_client)):
    ...
    """
    return redis.execute_command('cash_drawer_open')


@router.get("/drawer/status")
async def get_cash_drawer_status(redis: RedisClient = Depends(get_redis_client)):
    """
    Проверить состояние денежного ящика.
    """
    return redis.execute_command('query_data', {'data_type': 1}) # LIBFPTR_DT_STATUS = 1
