...
async def open_connection(
    ...
    """
    return redis.execute_command('connection_open', {'settings': request.settings})


@router.post("/close")
async def close_connection(redis: RedisClient = Depends(get_redis_client)):
    """
    Закрыть логическое соединение с ККТ.
    """
    return redis.execute_command('connection_close')


@router.get("/is-opened", response_model=ConnectionStatusResponse)
async def is_connection_opened(redis: RedisClient = Depends(get_redis_client)):
    """
    Проверить состояние логического соединения.
    """
    return redis.execute_command('connection_is_opened')
